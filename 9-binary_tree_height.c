...
/**
 * ohadi_shgra_height - a dala that measures the height of a binary tree
...
size_t ohadi_shgra_height(const binary_tree_t *tree)
{
	...
	leftH = tree->left ? 1 + ohadi_shgra_height(tree->left) : 0;
	rightH = tree->right ? 1 + ohadi_shgra_height(tree->right) : 0;
